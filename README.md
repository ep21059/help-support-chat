...
**FastAPI**、**Vue 3**、**PostgreSQL**、**Docker** で構築されたリアルタイム・カスタマーサポートチャットアプリケーション

## 機能
- **リアルタイムメッセージ**: WebSocketsを使用した即時双方向通信
...
- **日本時間 (JST) 対応**: すべてのタイムスタンプは日本時間で表示
- **永続化**: チャット履歴はPostgreSQLに保存
...
### 工夫点
1. **WebSockets**:
   - 訪問者は `/ws/visitor/{visitor_id}` に接続
   - オペレーターは `/ws/operator` に接続し、すべてのグローバルイベントを受信
   - 既読 (`messages_read`) 等の状態更新もWebSocket経由でリアルタイム配信

2. **REST API**:
   - `POST /api/conversations`: 新しい会話を作成
   - `POST /api/conversations/{visitor_id}/read`: メッセージを既読に
   - `POST /api/upload`: 画像ファイルのアップロード
   - `GET /api/conversations`: すべての会話履歴を取得

3. **タイムゾーン処理**:
   - バックエンドはUTC (`Z` suffix付き) で時刻を返し、フロントエンドがブラウザのロケール (JST) に変換して表示

### 今後の改善点
- **認証**: `/auth/login` ログイン機能の実装(現在は簡易実装)
